...
export default class TableDragSelect2 extends React.Component {
  static propTypes = {
    value: props => {
      const error = new Error(
        "Invalid prop `value` supplied to `TableDragSelect2`. Validation failed."
      );
    ...
    children: props => {
      if (TableDragSelect2.propTypes.value(props)) {
        return; // Let error be handled elsewhere
      }
      const error = new Error(
        "Invalid prop `children` supplied to `TableDragSelect2`. Validation failed."
      );
  ...
  render = () => {
    return (
      <table className={"table-drag-select " + this.props.uid}>
